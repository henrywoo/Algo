...
#define aInSetb(a,b) (b.find(a)!=b.end())
#define aNotInSetb(a,b) (b.find(a)==b.end())

    using namespace std;

    int GetLongestRange(int* head, int* tail){
        if (head==NULL || tail==NULL){
            ...
            unordered_set<int> ust(head,tail+1);
            unordered_set<int> ust2;
            int r=0;
            for (int* tmp= head; tmp <= tail; ++tmp){
                if (aNotInSetb(*tmp,ust2)){
                    if (tail-tmp+1 <= r){
                        ///optimization:if the size from tmp to tail(inclusive) is
                        ///less than r we are sure that r is the longest
                        ///range number!
                        return r;
                    }
                    ust2.insert(*tmp);
                    int count1=1;
                    while (aInSetb(*tmp+count1,ust)){//increment
                        ust2.insert(*tmp+count1);//forget
                        ++count1;
                    }
                    int count2=1;
                    while (aInSetb(*tmp-count2,ust)){//decrement
                        ust2.insert(*tmp-count2);//forget
                        ++count2;
                    }
                    r=max(r,1+count1+count2-2);
                }
            }
            return r;
        }
    ...
    int GetLongestRange(vector<int> &num){
    ...
    bool test_GetLongestRange(){
        ...
        {
            int *a=NULL;
            if (GetLongestRange(a,a)!=INVALIDPOINTER){
                return false;
            }
        }
        {
